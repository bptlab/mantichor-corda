...
class MainController(rpc: NodeRPCConnection) {
    ...
    fun getOrders() : ResponseEntity<List<StateAndRef<PizzaState>>> {
        return ResponseEntity.ok(proxy.vaultQueryBy<PizzaState>().states)
    }
    ...
    fun createOrder(request: HttpServletRequest): ResponseEntity<String> {
        ...
        return try {
            val signedTx = proxy.startTrackedFlow(::Initiator, pizzaAmount, pizzaPrice, otherParty).returnValue.getOrThrow()
    ...
    fun getMyOrders(): ResponseEntity<List<StateAndRef<PizzaState>>>  {
        val myOrders = proxy.vaultQueryBy<PizzaState>().states.filter { it.state.data.lieferant.equals(proxy.nodeInfo().legalIdentities.first()) }
